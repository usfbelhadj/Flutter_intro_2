...
void main() {
  ...
  for (int i = 1; i <= 5; i++) {
    test("Test 1: Testing Class Attributes    $i / 5", () async {
      ...
      expect([new_char.name, new_char.img, new_char.id], [name, img, id],
          reason: "Person attribute are Wrong");
    });
  }
  for (int i = 1; i <= 5; i++) {
    test("Test 1: Testing toJson method  $i / 5", () async {
      ...
      String img = faker.internet.httpUrl();
      int id = rng.nextInt(50);
      var mapp = {"name": name, "img": img, "id": id};
  ...
  for (int i = 1; i <= 5; i++) {
    test("Test 1: Testing from json  $i / 5", () async {
      ...
      expect([pers.name, pers.img, pers.id], [name, img, id],
          reason: "toJson method Should return $mapp");
    });
  }
}
